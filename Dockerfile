FROM eclipse-mosquitto:2.0.12 AS builder

RUN apk add make git gcc g++
RUN git clone --depth 1 --branch v2.0.12 https://github.com/eclipse/mosquitto.git mosquitto-src
...
FROM eclipse-mosquitto:2.0.12 AS prod
